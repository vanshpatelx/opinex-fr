...
//           <BarChart width={400} height={200} data={BuyChartData} layout="vertical" margin={{ left: -20 }} >
    ...
    const connectWebSocket = () => {
      ...
      socket.onmessage = (event) => {
        ...
        if (data.type === "orderbook") {
          ...
          while (buyOrders.length < 5) {
            const lastPrice = buyOrders.length > 0 ? buyOrders[buyOrders.length - 1].price : 100;
            buyOrders.push({ price: lastPrice - 0.5, quantity: 1, dummy: true });
          }
          ...
          while (sellOrders.length < 5) {
            const lastPrice = sellOrders.length > 0 ? sellOrders[sellOrders.length - 1].price : 101;
            sellOrders.push({ price: lastPrice - 0.5, quantity: 1, dummy: true });
          }
